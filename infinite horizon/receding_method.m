...
for loop_count=1:total_loop 
    ...
    [col_index_sigma] = sigma_col_index_new(A,B,k,k_present,Ha,1,T); %We set t as 1. For t=1 and specific Ha this is my X*
    [a] = choose_action(sigma,A,col_index_sigma);
    action_p1=[action_p1 a];

    %Choose player 2's action b_1 accoriding to the strategy tau
    [col_index_tau] = tau_col_index_new(A,B,l,l_present,Hb,1,T); %We set t as 1. For t=1 and specific Hb this is my Y*
    ...
    while stage_counter<=N
        ...
        [col_index_tau_plus] = tau_col_index_new(A,B,l,l_present,Hb,1,T); %For t=1 and specific Hb this is my Y*. In this case no matter what the Hb is, it is not using it as t=1.
        ...
        [col_index_sigma_plus] = sigma_col_index_new(A,B,k,k_present,Ha,1,T); %For t=1 and specific Ha this is my X*. In this case no matter what the Ha is, it is not using it as t=1.
